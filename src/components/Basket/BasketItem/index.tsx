...
import { setBasketAmount, setBasketRemove } from '../../../store/slices/basketSlice';
...
function BasketItem({ elem }: IProps) {
	...
	const [count, setCount] = useState<number>(elem.count);
	const price: number = getNewPrice(elem.count, elem.price);
	const removeBasket = () => {
		dispatch(setBasketRemove(elem.id));
	};
	useEffect(() => {
		const newPrice = Math.floor(count * price);
		const newElem = { ...elem, count, price: newPrice };
		dispatch(setBasketAmount({ id: elem.id, elem: newElem }));
	}, [count]);
	return (
		<li className='basket-item'>
			<div className='basket-item__left'>
				<div className='basket-item__images'>
					<img src={elem.image[0]} alt='product' />
				</div>
				<div className='basket-item__text'>
					<div className='basket-item__info'>
						<h5 className='basket-item__name'>{elem.name}</h5>
						<p className='basket-item__desc'>{elem.desc.substring(0, 73).trim() + '...'}</p>
					</div>
			...
			<Calculator amount={count} setAmount={setCount} />
			{elem.cashback && <Cashback num={elem.cashback} />}
